...
Leaflet is a fast, responsive web app. No installation is needed, though you *can* add it as a PWA.
...
1. Clone the repository `git clone https://tangled.org/leaflet.pub/leaflet.git`
   1. If using WSL, it's recommended to install in the native file structure rather than in a mounted Windows file structure (ie. prefer installing at `~/code/leaflet` rather than `/mnt/c/code/leaflet`)
2. Install the dependencies: `npm install`
3. Install the Supabase CLI:
   - **macOS:** `brew install supabase/tap/supabase`
   - **Windows:** `scoop bucket add supabase https://github.com/supabase/scoop-bucket.git && scoop install supabase`
   - **Linux:** Use Homebrew or download packages from the [releases page](https://github.com/supabase/cli/releases)
...
1. Start the local Supabase stack: `npx supabase start`
   - The first run takes longer while Docker images download
...
2. Copy the `.env` file example to `.env.local` and update it with your local values from the previous step:
...
1. Apply migrations to your local database:
   - First-time setup: `npx supabase db reset` (resets the database and applies all migrations)
...
- Add the `--no-backup` flag to reset the database on the next start
...
- Persisting articles on a fresh install over a fresh DB are usually due to stale Replicache entries. To clear them, open your browser DevTools and delete Replicache entries, usually under IndexedDB storage
- Supabase settings will get cached in `.next`; if you change where you're pointing your Supabase connections, you may need to delete the `.next` folder (it will rebuild the next time you start the app)
...
- [React](https://react.dev/) and [Next.js](https://nextjs.org/) for UI and app framework
- [Supabase](https://supabase.com/) for the DB and storage layer
- [Replicache](https://replicache.dev/) for the real-time data sync layer
