...
export function useBlockMouseHandlers(props: Block) {
  ...
  // Handlers read everything through a ref so their identity is stable across
  // renders (a `props` dep would recreate them every render).
  let latest = useRef({
    props,
    write: entity_set.permissions.write,
    isMobile,
    rep,
  });
  latest.current = { props, write: entity_set.permissions.write, isMobile, rep };

  let onMouseDown = useCallback((e: MouseEvent) => {
    let { props, write, isMobile } = latest.current;
  ...
  let onMouseEnter = useCallback(async (e: MouseEvent) => {
    let { props, write, isMobile, rep } = latest.current;
    if (isMobile && isScrolling) return;
    if (!write) return;
