    ...
    <>
      <div className="pubDashHeader flex flex-row gap-2 w-full justify-between border-b border-border text-secondary items-center ">
        <div className="max-w-full w-[1000px] h-full ">
          <div className="flex gap-2 h-fit  py-0.5 pl-1 pr-2 w-fit  rounded-md sm:bg-[rgba(var(--bg-leaflet),0.8)]">
            {props.icon && (
              <div
...
function Tab(props: { name: string; selected: boolean; onSelect: () => void }) {
  return (
    <div
      className={`pubTabs border bg-[rgba(var(--bg-leaflet),var(--bg-page-alpha))] border-b-0 px-2 pt-1 pb-0.5 rounded-t-md border-border hover:cursor-pointer ${props.selected ? "text-accent-1 font-bold -mb-[1px]" : ""}`}
