...
export function PublicationHomeLayout(props: {
  ...
  return (
    ...
    >
      <div
        ref={!props.showPageBackground ? null : ref}
        className={`pubContent sm:max-w-(--page-width-units) max-w-full w-[1000px] mx-auto px-3 sm:px-4 py-5  ${props.showPageBackground ? "overflow-auto h-full bg-[rgba(var(--bg-page),var(--bg-page-alpha))] border border-border rounded-lg" : "h-fit"}`}
      >
        {props.children}
      </div>
    </div>
