...
// The page and its generateMetadata both need the token and the whole fact set;
// these are the two heaviest queries in the app, so they run once per request.
const getLeafletData = cache((token_id: string) =>
  get_leaflet_data.handler({ token_id }, { supabase: supabaseServerClient }),
);
const getInitialFacts = cache(async (root: string) => {
  let { data } = await supabaseServerClient.rpc("get_facts", { root });
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  let initialFacts = await getInitialFacts(rootEntity);
