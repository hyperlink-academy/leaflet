...
export const useOpenPages = (): OpenPage[] => {
  ...
  const pageParam = useClientSearchParams().get("page");
...
export const useInitializeOpenPages = () => {
  ...
  useEffect(() => {
    ...
    if (!state.initialized) {
      // Read the live URL rather than a rendered value: the hydration render
      // deliberately sees no search params (see useClientSearchParams), and
      // initializing from that snapshot would discard a ?page= deep link
      // permanently, since initialization only ever happens once.
