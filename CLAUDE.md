...
- **Custom domain settings**: Custom domains are listed, verified, and assigned in three disconnected surfaces (profile `settings/domains`, publication dashboard `PubDomainSettings.tsx`, editor share menu `ShareOptions/DomainOptions.tsx`) that each hand-roll their own row UI over the same data. When changing domain row UI or behavior in any one of them, use the `domain-settings` skill (`.claude/skills/domain-settings/`), which lists the shared pieces and requires asking whether the change applies to the other two.
- **Publication settings**: A setting in the General tab passes through several hand-maintained explicit field lists (both lexicons, `normalizePublication`, both builders in `buildPublicationRecord.ts`, four spots in `SettingsContent.tsx`, `createPublication.ts`) — none of them spread, so a field missing from one is dropped with no type error. When adding or changing a setting there, use the `publication-settings` skill (`.claude/skills/publication-settings/`), which walks the round trip and how to verify it saves and applies.
