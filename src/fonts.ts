...
// Name of the metric-matched local fallback @font-face generated by
// FontLoader (see src/fontMetricFallbacks.ts). Included in the font stack
// unconditionally: browsers skip unknown font family names, so it's harmless
// in contexts where the @font-face isn't defined (emails, client-side theme
// changes, fonts without known metrics).
export function getMetricFallbackName(font: FontConfig): string | null {
  if (font.type !== "google") return null;
  return `${font.fontFamily} Fallback`;
}
...
export function getFontFamilyValue(font: FontConfig): string {
  const family = font.fontFamily.includes(" ")
    ? `'${font.fontFamily}'`
    : font.fontFamily;
  const metricFallback = getMetricFallbackName(font);
  return [
    family,
