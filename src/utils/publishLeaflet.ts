import { invalidateIdentitySlicesForDid } from "src/identitySlices";
...
// Publish a draft leaflet as a document record, acting as `actorDid`. This is
...
async function publish({
  ...
  showInDiscover,
  rkey: requestedRkey,
}: PublishLeafletArgs & { actorDid: string }): Promise<PublishResult> {
  ...
  if (publication_uri) {
    ...
    let isOwner = data.identity_did === actorDid;
    if (!isOwner && !(await isConfirmedContributor(publication_uri, actorDid)))
  ...
  const rkey = existingDocUri
    ? new AtUri(existingDocUri).rkey
    : requestedRkey || TID.nextStr();
  ...
  if (publication_uri) {
    ...
    await supabaseServerClient
      ...
      .upsert(
        { leaflet: leaflet_id, contributor_did: actorDid },
  ...
  await invalidateIdentitySlicesForDid(actorDid, ["leaflets"]);
