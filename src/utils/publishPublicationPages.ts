...
// home route "/".
export const routeShape = /^\/[a-z0-9-]+$/;

// Publish a publication's draft leaflet in one go: page records, nav rows,
// and theme, writing to `actorDid`'s PDS with their stored session. Callers
// are responsible for having authorized `actorDid`. Re-running converges:
// rkeys are stable per page entity, rows are upserts, and stale state is
// re-diffed every time.
export async function publishPublicationPages({
  ...
  const { data: publication } = await supabaseServerClient
    ...
    .single();
  if (!publication || publication.identity_did !== actorDid)
  ...
  const sessionResult = await restoreOAuthSession(actorDid);
  if (!sessionResult.ok) {
  ...
  await supabaseServerClient
    ...
    .eq("uri", publication_uri);
  await invalidateIdentitySlicesForDid(actorDid, ["publications", "leaflets"]);
