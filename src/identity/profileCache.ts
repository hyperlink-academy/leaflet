...
export type DebugCacheEntry =
  | { status: "hit"; profile: Profile | null; updatedAt: number; ageMs: number; expired: boolean }
  | { status: "miss" }
  | { status: "malformed"; raw: string }
  | { status: "disabled" };

export async function debugReadCache(
  dids: string[],
): Promise<Map<string, DebugCacheEntry>> {
  const out = new Map<string, DebugCacheEntry>();
  if (!redisClient) {
    for (const did of dids) out.set(did, { status: "disabled" });
    return out;
  }
  if (dids.length === 0) return out;

  const raw = await redisClient.mget(...dids.map(profileKey));
  const now = Date.now();
  raw.forEach((value, i) => {
    const did = dids[i];
    if (!value) {
      out.set(did, { status: "miss" });
      return;
    }
    try {
      const entry = JSON.parse(value) as CachedEntry;
      ...
      out.set(did, {
        ...
        ageMs,
        expired: ageMs > MAX_TTL * 1000,
      });
    } catch {
      out.set(did, { status: "malformed", raw: value });
    }
  });
  return out;
}

export async function debugFetchProfiles(
  dids: string[],
): Promise<Map<string, Profile | null>> {
  return fetchProfiles(dids);
}
